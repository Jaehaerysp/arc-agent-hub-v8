 ...
 * same address always renders identically. Per Design Vision §13, this
 * stands in for a photo avatar or robot clip-art — it's the product's
 * "visual hash" trust signal.
 *
 * Note: this intentionally mirrors
 * src/features/dashboard/components/AgentIdentityMark.jsx. The Dashboard
 * module is out of scope for this milestone (do-not-modify), so it can't be
 * refactored to import a shared version without touching a locked file —
 * this shared copy lets Marketplace/Profile use the identical visual
 * language without that edit. See ARCHITECTURE_NOTES in the m4 summary.
 */
export function AgentIdentityMark({ seed, size = 40 }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" className="agent-mark" role="img" aria-label="Agent identity mark">
