...
import { Panel, FieldGroup, Input, Textarea, Select, Badge, Button } from '../../../ui/design-system'
...
/**
 * Give Feedback — moved from `ReputationPage.jsx` verbatim in business
 * logic (same `giveFeedback` call, same args, same feedback-hash
 * construction). The only addition is passing `feedbackType` through to
 * `addActivity`'s metadata (see `activityMeta` below) so the Trust
 * Analytics "Network Distribution" chart has real data to chart — this
 * is a log-metadata addition only, not a change to the contract call.
 */
export function FeedbackFormPanel() {
  ...
  return (
    <Panel
      ...
      title="Give Feedback"
      subtitle="Submit a review via giveFeedback"
    ...
    >
      ...
      <div className="tv7-score-block field-group">
        <div className="tv7-score-header">
          <label>Score (1–10)</label>
          <div className="tv7-score-display">
            <span className="tv7-score-number">{score}</span>
            <span className="tv7-score-max">/10</span>
            <span className="tv7-score-emoji">{SCORE_LABELS[score]}</span>
          </div>
        </div>
        <input
          className="slider"
      ...
      <Button variant="primary" block loading={loading} onClick={handleSubmit} disabled={!signer || submitted}>
